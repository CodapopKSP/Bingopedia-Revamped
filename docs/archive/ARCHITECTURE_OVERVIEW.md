...
- **Top-Level Structure (implemented)**
  - `src/`
    ...
    - `features/game/`
      - `useGameState.ts` – Main game state hook managing game lifecycle, timer, matches, win detection.
...
- **Key Frontend Responsibilities**
  ...
  - Own the **timer and click counter** behavior, including pausing during loads.
    - Location: `features/game/useGameState.ts` → timer managed via `useEffect` with `articleLoading` state.
...
### Timer Behavior
- Starts when first article navigation occurs (not on game start).
- Pauses automatically when `articleLoading` is true.
- Resumes when article load completes.
...
- **MongoDB Usage**
  ...
  - Minimal schema (mirrors existing implementation):
    ...
    - `history`: string[] (article titles in visit order).
    - `createdAt`: Date.
  - Index: `{ score: -1 }` (and potentially `createdAt` later if needed).
