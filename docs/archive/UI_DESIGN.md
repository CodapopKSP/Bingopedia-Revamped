...
**Last Updated**: Based on implementation review of the new TypeScript/React codebase.
...
[User clicks on a leaderboard entry (username)]
  ↓ Game Details Modal opens
[Modal shows: bingo squares, stats, full history]
  ↓ User clicks on a bingo square
[Article Summary Modal opens (read-only)]
  ↓ User closes summary modal
[Game Details Modal still visible]
...
- Bingo squares in game details can be clicked to see summaries
